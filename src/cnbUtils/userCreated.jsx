// 导入indexedDB工具函数
import {
  saveUserCreatedIssues,
  getUserCreatedIssuesFromCache,
  getCacheMetadata
} from './indexedDB';
 ...
 * @returns {Promise<Array>} 用户创建的issue列表
 */
export const getUserCreatedIssues = async ({
  ...
  try {
    ...
    if (!forceRefresh) {
      try {
        const cachedData = await getUserCreatedIssuesFromCache(todoStatus, page, pageSize);
        if (cachedData.issues.length > 0) {
          console.log('从缓存获取用户创建内容成功');
    ...
    // 从网络获取数据
    const response = await fetch(`${import.meta.env.VITE_CNBCOOKIE_API_URL}/user/dashboard/mine_create/issue?todoStatus=${todoStatus}&page=${page}&page_size=${pageSize}`, {
      method: 'GET',
      headers: {
    ...
    if (issues.length > 0) {
      try {
        await saveUserCreatedIssues(issues, todoStatus);
        console.log('用户创建内容已保存到缓存');
      } catch (saveError) {
    ...
    console.error('获取用户创建内容失败:', error);
...
const fetchAndUpdateCache = async ({ todoStatus, page, pageSize, session }) => {
  try {
    // 检查缓存是否需要更新（比如超过一定时间）
    const metadata = await getCacheMetadata(`user_created_${todoStatus}`);
    const now = new Date();
