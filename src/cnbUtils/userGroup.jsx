...
export const getUserGroups = async ({
  ...
  try {
    ...
    if (!forceRefresh) {
      try {
        const cachedData = await getUserGroupsFromCache(page, pageSize);
    ...
    // 从网络获取数据
    const response = await fetch(`${import.meta.env.VITE_CNBCOOKIE_API_URL}/user/groups?page=${page}&page_size=${pageSize}`, {
      method: 'GET',
      headers: {
 ...
 */
export const formatGroupsForDisplay = (groups) => {
...
/**
 * 获取访问权限的标签
 * @param {string} accessRole - 访问权限
 * @returns {string} 对应的中文标签
 */
export const getAccessRoleLabel = (accessRole) => {
  const accessRoleMap = {
    'Owner': '所有者',
    'Maintainer': '维护者',
    'Developer': '开发者',
  ...
  return accessRoleMap[accessRole] || accessRole;
};
