...
const Info = () => {
  ...
  // 从浏览器存储中获取分页参数，默认值为1和20
  const [pageNum, setPageNum] = useState(() => {
    const savedSettings = localStorage.getItem('settingsData');
    return savedSettings ? JSON.parse(savedSettings).pageNum : 1;
  });
  
  const [sizeNum, setSizeNum] = useState(() => {
    const savedSettings = localStorage.getItem('settingsData');
    return savedSettings ? JSON.parse(savedSettings).sizeNum : 20;
  });

  // 当 pageNum 或 sizeNum 变化时，保存到本地存储
  useEffect(() => {
  ...
  const loadComments = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/${baseRepo}/-/issues/${number}/comments?page=${pageNum}&page_size=${sizeNum}`, {
        method: 'GET',
        headers: {
          'accept': 'application/json',
          'Authorization': `Bearer ${cnbToken}`
